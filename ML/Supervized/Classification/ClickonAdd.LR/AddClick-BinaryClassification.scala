...
val logregdataall = data.select(data("Clicked on Ad").as("label"), $"Daily Time Spent on Site", $"Area Income", $"Age", $"Daily Internet Usage", $"Timestamp", $"Male")
val logregdf = logregdataall.na.drop()
val logregdata = logregdataall.withColumn("hour", hour(col("Timestamp")))
