...
val data = spark.read.option("header","true").option("inferSchema","true").csv("Cancer_Data")
...
val data_features = assembler.transform(data).select($"features")
...
// Compute summary statistics by fitting the StandardScaler.
val scalerModel = scaler.fit(data_features)
...
// Normalize each feature to have unit standard deviation.
val scaledFeatures = scalerModel.transform(data_features)
